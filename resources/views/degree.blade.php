...
<head>
    ...
    <style>
        ...
        /* Change the text color inside the search input */
        .select2-search__field {
            color: #2d3748; /* Tailwind text-gray-700 */
            background-color: #f7fafc; /* Tailwind bg-gray-50 */
        }

        /* Change the placeholder color inside the search input */
        ...
        /* Dark mode text color for Select2 options */
        .dark .select2-results__option {
            color: #edf2f7; /* Tailwind text-white for dark mode */
        }

        /* Change the highlighted text color in dark mode */
        .dark .select2-results__option--highlighted {
            color: #f56565; /* Tailwind text-red-400 for highlighted option in dark mode */
        }
    </style>
</head>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        dropdownItems.forEach((item) => {
            item.addEventListener("click", function (e) {
                ...
                const selectedValue = this.getAttribute("data-value"); // Get value
                const selectedId = this.getAttribute("data-id"); // Get ID

                dropdownButton.childNodes[0].textContent = selectedValue; // Update button text
                searchInput.value = ""; // Clear search input
                dropdownItems.forEach((item) => (item.style.display = "")); // Reset list
                dropdownMenu.classList.add("hidden"); // Hide dropdown

                // Store selected values (Optional)
                ...
                // Manually trigger change event on #selectedId
